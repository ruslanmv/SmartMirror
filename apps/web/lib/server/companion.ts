 ...
 */
export type HandoffPurpose = "body" | "garment";

interface Ticket {
  v: 1;
  sid: string;
  /** body = one photo for try-on; garment = any number of clothes photos (closet scan). */
...
export async function openTicket(
  raw: unknown,
): Promise<{ sid: string; purpose: HandoffPurpose; session: SessionData | null } | null> {
  ...
  return { sid: t.sid, purpose: t.purpose === "garment" ? "garment" : "body", session };
}
